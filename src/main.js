...
let selectedTimeControl = TIME_CONTROL_IDS.FIFTEEN_MINUTES;
...
const GAME_END_ROYAL_BADGE_DELAY_MS = 2000;
let gameEndResultOverlayTimer = null;
...
const noopAdManager = {
  ...
  showBanner() {},
  hideBanner() {},
  refreshBannerForOrientation() {},
  prepareInterstitial() {},
  showInterstitial() {}
};
let AdManager = noopAdManager;

async function loadNativeAdManager() {
  if (!Capacitor.isNativePlatform()) {
    return noopAdManager;
  }

  try {
    const module = await import('./utils/AdManager.js');
    AdManager = module.AdManager || noopAdManager;
    return AdManager;
  } catch (error) {
    console.warn('AdMob: Native ad manager could not be loaded. Continuing without ads.', error);
    AdManager = noopAdManager;
    return AdManager;
  }
}

function initializeAdManagerSafely() {
  if (!Capacitor.isNativePlatform() || screenshotMode.enabled) {
    AdManager = noopAdManager;
    return;
  }

  loadNativeAdManager().then((manager) => {
...
const AI_BOT_DEFEATED_KEY = 'timur_ai_bot_defeated_v1';
...
const btnOpenSetupSheet = document.getElementById('btn-open-setup-sheet');
...
const btnToggleOrientationSetting = document.getElementById('btn-toggle-orientation-setting');
...
const orientationSettingToggleState = document.getElementById('orientation-setting-toggle-state');
...
const matchSettingsSheet = document.getElementById('match-settings-sheet');
const setupTabButtons = document.querySelectorAll('.setup-tab');
const setupTabPanels = document.querySelectorAll('.setup-tab-panel');
const matchSetupSummary = document.getElementById('match-setup-summary');
const matchSetupSummaryMeta = document.getElementById('match-setup-summary-meta');
...
function updateOrientationSettingUI() {
  if (!btnToggleOrientationSetting || !orientationSettingToggleState) return;

  const isLandscape = orientationManager.get() === 'landscape';
  orientationSettingToggleState.textContent = i18n.t(
    isLandscape ? 'orientation.switch_to_portrait' : 'orientation.switch_to_landscape'
  );
}

function forcePortraitOutsideGame({ refreshBanner = true } = {}) {
  if (orientationManager.get() !== 'landscape') return;

  const result = orientationManager.set('portrait');
  Promise.resolve(result).then(() => {
    updateOrientationSettingUI();
    if (refreshBanner) {
      AdManager.refreshBannerForOrientation();
    }
  });
}
...
function readDefeatedAiBotMap() {
  try {
    const parsed = JSON.parse(localStorage.getItem(AI_BOT_DEFEATED_KEY) || '{}');
    return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : {};
  } catch (error) {
    console.warn('[AiBotProgress] Could not read defeated bot progress.', error);
    return {};
  }
}

function writeDefeatedAiBotMap(map) {
  try {
    localStorage.setItem(AI_BOT_DEFEATED_KEY, JSON.stringify(map));
  } catch (error) {
    console.warn('[AiBotProgress] Could not persist defeated bot progress.', error);
  }
}

function isPlayerWinAgainstAiBotRecord(record) {
  return Boolean(
    isAIBotId(record?.aiBotId)
    && (record.winner === COLORS.WHITE || record.winner === COLORS.BLACK)
    && record.winner === record.playerColor
  );
}

function getDefeatedAiBotMap() {
  const defeatedMap = readDefeatedAiBotMap();

  getMatchHistoryRecords().forEach((record) => {
    if (!isPlayerWinAgainstAiBotRecord(record) || defeatedMap[record.aiBotId]) return;

    defeatedMap[record.aiBotId] = {
      botId: record.aiBotId,
      wonAt: record.finishedAt || record.createdAt || null,
      level: record.aiBotLevel || null,
      stars: record.aiBotStars || null,
      source: 'history'
    };
  });
...
function markAiBotDefeatedByPlayer({
  state = gameState,
  winner = gameState?.winner,
  resultType = resolveGameEndResultType(gameState)
} = {}) {
  const botId = state?.aiBotId || currentGameRecordMeta?.aiBotId || null;
  const playerColor = state?.playerColor || currentGameRecordMeta?.localColor || COLORS.WHITE;

  if (!isAIBotId(botId) || !(winner === COLORS.WHITE || winner === COLORS.BLACK) || winner !== playerColor) {
    return false;
  }
  ...
  defeatedMap[botId] = {
    ...
    level: state?.aiBotLevel || currentGameRecordMeta?.aiBotLevel || bot.level,
    stars: state?.aiBotStars || currentGameRecordMeta?.aiBotStars || bot.stars,
    resultType,
    moveCount: state?.moveHistory?.length || 0,
    source: 'game_end'
  };
  writeDefeatedAiBotMap(defeatedMap);
  return true;
}
...
function getSetupCardLabel(cards, dataKey, value) {
  const card = Array.from(cards || []).find(item => item?.dataset?.[dataKey] === value);
  return card?.querySelector('h3')?.textContent?.trim() || value || '-';
}

function updateMatchSetupSummary() {
  if (!matchSetupSummary && !matchSetupSummaryMeta) return;

  const formationLabel = getSetupCardLabel(formationCards, 'formation', selectedFormation);
  const personaLabel = getSetupCardLabel(aiPersonaCards, 'aiPersona', selectedAiPersonaId);
  const timeLabel = getTimeControlLabel(selectedTimeControl);
  const difficultyLabel = getSetupCardLabel(difficultyCards, 'difficulty', selectedDifficulty);
  const colorLabel = getSetupCardLabel(playerColorCards, 'playerColor', selectedPlayerColor);

  if (matchSetupSummary) {
    matchSetupSummary.textContent = `${formationLabel} · ${personaLabel} · ${timeLabel}`;
  }

  if (matchSetupSummaryMeta) {
    matchSetupSummaryMeta.textContent = `${difficultyLabel} · ${colorLabel}`;
  }
}

function setSetupSheetOpen(isOpen) {
  if (!matchSettingsSheet) return;

  matchSettingsSheet.classList.toggle('hidden', !isOpen);
  btnOpenSetupSheet?.setAttribute('aria-expanded', String(isOpen));

  if (isOpen) {
    const activeTab = matchSettingsSheet.querySelector('.setup-tab.active') || setupTabButtons[0];
    activeTab?.focus?.();
  } else {
    btnOpenSetupSheet?.focus?.();
  }
}

function activateSetupTab(tabName = 'formation') {
  const targetTab = tabName || 'formation';

  setupTabButtons.forEach(button => {
    const isActive = button.dataset.setupTab === targetTab;
    button.classList.toggle('active', isActive);
    button.setAttribute('aria-selected', String(isActive));
    button.setAttribute('tabindex', isActive ? '0' : '-1');
  });

  setupTabPanels.forEach(panel => {
    const isActive = panel.dataset.setupPanel === targetTab;
...
function renderAiBotCards() {
  ...
  const defeatedBotMap = getDefeatedAiBotMap();
  ...
  Array.from(groupedByStars.entries())
    ...
    .forEach(([stars, models]) => {
      ...
      const tierStars = document.createElement('span');
      tierStars.className = 'ai-bot-tier-stars';
      tierStars.textContent = '★'.repeat(Number(stars));

      const tierText = document.createElement('span');
      tierText.className = 'ai-bot-tier-text';
      tierText.textContent = models[0]?.starsLabel || `${stars} yıldız`;

      tierLabel.append(tierStars, tierText);
      ...
      models.forEach((model) => {
        const defeatedRecord = defeatedBotMap[model.id] || null;
        const card = document.createElement('div');
        card.className = 'card ai-bot-card ai-bot-list-item';
        card.dataset.aiBot = model.id;
        if (defeatedRecord) {
          card.classList.add('is-defeated');
          card.dataset.defeated = 'true';
        }
        card.setAttribute('role', 'button');
        card.setAttribute('tabindex', '0');
        card.setAttribute('aria-label', defeatedRecord
          ? `${model.label} - ${i18n.t('ai.bot.defeated_aria')}`
          : model.label
        );

        const icon = document.createElement('i');
        icon.className = `${model.iconClass} card-icon icon-persona icon-bot`;
        ...
        if (defeatedRecord) {
          ...
          defeatedBadge.className = 'ai-bot-defeated-badge';
          defeatedBadge.innerHTML = `<i class="fas fa-trophy" aria-hidden="true"></i><span>${i18n.t('ai.bot.defeated_badge')}</span>`;
          card.appendChild(defeatedBadge);
        }
...
function finalizeCurrentGame({
  ...
  if (markAiBotDefeatedByPlayer({ state: gameState, winner, resultType })) {
    renderAiBotCards();
  }

  saveCurrentMatchHistory(gameState);
  void queueGameRecordUpload(gameState);
  const shouldDelayResultOverlay = revealGameEndRoyalBadges(winner);
  scheduleGameEndResultOverlay({
    delayMs: shouldDelayResultOverlay ? GAME_END_ROYAL_BADGE_DELAY_MS : 0,
    state: gameState
  });
...
async function toggleOrientationFromSettings() {
  if (!btnToggleOrientationSetting) return;
  if (btnToggleOrientationSetting.disabled) return;

  btnToggleOrientationSetting.disabled = true;
  try {
    await orientationManager.toggle({ animate: true });
    updateOrientationSettingUI();
    analytics?.track?.('orientation_toggled', {
      mode: orientationManager.get(),
      source: 'game_settings'
    });
  } catch (error) {
    console.warn('Orientation setting toggle failed:', error);
  } finally {
    btnToggleOrientationSetting.disabled = false;
  }
}
...
function syncGameHud(advantageContext = {}) {
  ...
  if (topPlayerAvatarIcon) {
    topPlayerAvatarIcon.className = isOnlineMatch
      ? 'fas fa-user icon-persona'
      : 'fas fa-robot icon-persona icon-bot';
  }
...
function closeGameEndResultOverlay() {
  clearGameEndResultOverlayTimer();
  gameEndResultOverlayElement?.classList.add('hidden');
}

function clearGameEndResultOverlayTimer() {
  if (!gameEndResultOverlayTimer) return;
  clearTimeout(gameEndResultOverlayTimer);
  gameEndResultOverlayTimer = null;
}

function revealGameEndRoyalBadges(winner = gameState?.winner) {
  if (!gameState || !(winner === COLORS.WHITE || winner === COLORS.BLACK) || !boardRenderer) {
    return false;
  }

  gameState.showRoyalResultBadges = true;
  gameState.royalResultBadgeWinner = winner;
  boardRenderer.render();
  return true;
}

function scheduleGameEndResultOverlay({ delayMs = 0, state = gameState } = {}) {
  clearGameEndResultOverlayTimer();
  ...
  gameEndResultOverlayTimer = setTimeout(() => {
    gameEndResultOverlayTimer = null;
...
function renderGameEndResultOverlay() {
  ...
  const reasonLabel = i18n.t(`game_end.reason.${resultType}`);
  const detailedResultLabel = resultType === 'timeout_win'
    ? reasonLabel
  ...
  const resultBox = gameEndResultOverlayElement.querySelector('.game-end-result-box');
  resultBox?.classList.remove('is-win', 'is-loss', 'is-draw');
  resultBox?.classList.add(badgeClass);
  ...
  if (gameEndResultTitle) {
    gameEndResultTitle.textContent = i18n.t(badgeKey);
  }

  if (gameEndResultSubtitle) {
    gameEndResultSubtitle.textContent = detailedResultLabel === reasonLabel
      ? reasonLabel
      : `${detailedResultLabel}. ${reasonLabel}`;
  }
...
function init() {
  ...
  orientationManager.init({ forcePortrait: true });
  ...
  document.addEventListener('adbannerlayoutchange', () => {
    requestBoardScale();
    requestBoardScale(180);
  });
  applyInitialLocale();
  setupVersionDisplay();
  ...
  renderAiBotCards();
  applySetupSelection(getDefaultMatchSetup());
  activateSetupTab('formation');
...
function renderMatchHistoryList() {
  ...
  records.forEach((record) => {
    ...
    const hasColorWinner = record.winner === COLORS.WHITE || record.winner === COLORS.BLACK;
    const resultTone = record.isDraw || record.resultType === 'citadel_draw' || !hasColorWinner
      ? 'is-draw'
      : (record.winner === record.playerColor ? 'is-win' : 'is-loss');
    card.classList.add(resultTone);

    const header = document.createElement('div');
    header.className = 'match-history-card-header';

    const titleWrap = document.createElement('div');
    titleWrap.className = 'match-history-title-wrap';
    ...
    meta.textContent = [
      ...
      record.aiBotLabel
        ? `${i18n.t('history.bot')}: ${record.aiBotLabel}`
    ...
    summaryGrid.className = 'match-history-summary-grid';
    [
      [i18n.t('history.difficulty'), record.aiBotLabel
        ? `${i18n.t('history.level')}: ${record.aiBotLevel || '-'}`
        : (record.difficultyLabel || record.difficulty || '-')],
      [i18n.t('history.time_control'), getTimeControlLabel(record.timeControl)],
      [i18n.t('history.move_count'), record.moveCount || record.moves?.length || 0],
      [i18n.t('history.analysis'), record.analysisSummary ? getHistoryAnalysisLabel(record.analysisSummary) : i18n.t('history.analysis_pending')]
    ].forEach(([label, value]) => {
      const item = document.createElement('span');
      item.className = 'match-history-summary-chip';
    ...
    resultPill.className = `match-history-result-pill ${resultTone}`;
    resultPill.textContent = record.resultLabel || i18n.t('analysis.result.ongoing');

    header.append(titleWrap, resultPill);
...
function setupEventListeners() {
  ...
  btnQuickStart?.addEventListener('click', () => {
    ...
    startMatchFromSetup(getDefaultMatchSetup(), 'toast.quick_start');
  });
  ...
  if (btnCopyCode) {
    btnCopyCode.addEventListener('click', async () => {
      ...
      try {
        ...
        btnCopyCode.innerHTML = `<i class="fas fa-check icon-system icon-action" aria-hidden="true"></i> <span>${successText}</span>`;
  ...
  if (btnToggleOrientationSetting) {
    btnToggleOrientationSetting.addEventListener('click', () => {
      toggleOrientationFromSettings();
    });
  }
  ...
  document.getElementById('board-theme-group')?.addEventListener('click', (e) => {
    ...
    if (!btn) return;
    const pair = themeManager.setBoardThemePair(btn.dataset.boardTheme);
    PieceRenderer.refreshAllPieces();
    updateThemeUI();
    analytics.track('board_theme_changed', { theme: pair.boardTheme, pieceSkin: pair.pieceSkin });
  });
  ...
  document.getElementById('piece-skin-group')?.addEventListener('click', (e) => {
    ...
    if (!btn) return;
    const pair = themeManager.setPieceSkinPair(btn.dataset.pieceSkin);
    PieceRenderer.refreshAllPieces();
    updateThemeUI();
    analytics.track('piece_skin_changed', { skin: pair.pieceSkin, theme: pair.boardTheme });
  });
  ...
  btnOpenSetupSheet?.addEventListener('click', () => {
    audioManager.playClickSound();
    activateSetupTab(matchSettingsSheet?.querySelector('.setup-tab.active')?.dataset.setupTab || 'formation');
  ...
  matchSettingsSheet?.addEventListener('keydown', (event) => {
    if (event.key === 'Escape') {
  ...
  setupTabButtons.forEach(button => {
    button.addEventListener('click', () => {
      audioManager.playClickSound();
      activateSetupTab(button.dataset.setupTab || 'formation');
    });
  });
...
async function startGame(formation, difficulty, isScripted = false, onlineSnapshot = null, options = {}) {
  ...
  gameState.showRoyalResultBadges = false;
  gameState.royalResultBadgeWinner = null;
  gameState.analysisStatus = 'idle';
  gameState.analysisReport = null;
  gameState.analysisPromise = null;
  applyOnlineMatchMetadata(gameState);
...
function loadLesson(index) {
  ...
  // Render lesson content. Quick action cards were removed from the guide so
  // every tab opens directly into readable lesson material.
  contentEl.innerHTML = `
