...
void Position::set_check_info(StateInfo* si) const {
  ...
  const Square whiteRoyalSq = timur_effective_royal_square(*this, WHITE);
  const Square blackRoyalSq = timur_effective_royal_square(*this, BLACK);

  si->blockersForKing[WHITE] = slider_blockers(pieces(BLACK), whiteRoyalSq, si->pinners[BLACK], BLACK);
  si->blockersForKing[BLACK] = slider_blockers(pieces(WHITE), blackRoyalSq, si->pinners[WHITE], WHITE);

  Square ksq = sideToMove == WHITE ? blackRoyalSq : whiteRoyalSq;
...
void Position::set_state(StateInfo* si) const {
  ...
  const Square ownRoyalSq = timur_effective_royal_square(*this, sideToMove);
  si->checkersBB = ownRoyalSq != SQ_NONE ? attackers_to(ownRoyalSq, ~sideToMove) : Bitboard(0);
  si->move = MOVE_NONE;
  ...
  if (var->variantTemplate == "timur")
  {
      for (Color c : {WHITE, BLACK})
      {
          if (si->timurCitadelExchangeUsed[c])
              si->key ^= timur_citadel_exchange_used_key(c);
          if (si->timurRoyalSwapUsed[c])
              si->key ^= timur_royal_swap_used_key(c);
      }
      ...
      for (int sq = SQ_A1; sq <= SQ_MAX; ++sq)
      {
          ...
          Piece pc = piece_on(s);
          int stage = si->timurPawnOfPawnsStage[s];
          if (stage > TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL
              && pc != NO_PIECE
              && type_of(pc) == PAWN)
              si->key ^= timur_pawn_of_pawns_stage_key(color_of(pc), s, stage);
      }
  }
}
...
bool Position::legal(Move m) const {
  ...
  const Square royalSq = timur_effective_royal_square(*this, us);
  const bool movingEffectiveBackupRoyal = timur_is_effective_backup_royal_square(*this, us, from);
  ...
  if (movingEffectiveBackupRoyal)
      return !attackers_to(to, occupied, ~us);

  // Return early when without a royal target.
  if (royalSq == SQ_NONE)
      return true;
  ...
  // A non-king move is legal if the current effective royal is not under attack after the move.
  return !(attackers_to(royalSq, occupied, ~us, janggiCannons) & ~SquareBB[to]);
}
...
bool Position::pseudo_legal(const Move m) const {
  ...
  if (checkers() && !(checkers() & non_sliding_riders()))
  {
      ...
      if (type_of(pc) != KING && !movingEffectiveBackupRoyal)
      {
          ...
          if (  !(between_bb(royalSq, lsb(checkers())) & to)
              || ((LeaperAttacks[~us][type_of(piece_on(checksq))][checksq] & royalSq) && !(checkers() & to)))
              return false;
      }
      // In case of royal moves under check we have to remove the royal so as to catch
...
bool Position::gives_check(Move m) const {
  ...
  Square from = from_sq(m);
  Square to = to_sq(m);
  const Square targetRoyalSq = timur_effective_royal_square(*this, ~sideToMove);

  // No check possible without an opposing royal target.
  if (targetRoyalSq == SQ_NONE)
      return false;
      ...
      && !((var->petrifyOnCaptureTypes & type_of(moved_piece(m))) && capture(m)))
  {
      ...
      if (pt == JANGGI_CANNON)
      {
          if (attacks_bb(sideToMove, pt, to, occupied) & attacks_bb(sideToMove, pt, to, occupied & ~janggiCannons) & targetRoyalSq)
              return true;
      }
      else if (AttackRiderTypes[pt] & (HOPPING_RIDERS | ASYMMETRICAL_RIDERS))
      {
          if (attacks_bb(sideToMove, pt, to, occupied) & targetRoyalSq)
              return true;
      }
  ...
  if (  ((type_of(m) != DROP && (blockers_for_king(~sideToMove) & from)) || (non_sliding_riders() & pieces(sideToMove)))
      && attackers_to(targetRoyalSq, occupied, sideToMove, janggiCannons) & occupied)
      return true;

  // Is there a check by gated pieces?
  if (    is_gating(m)
      && attacks_bb(sideToMove, gating_type(m), gating_square(m), (pieces() ^ from) | to) & targetRoyalSq)
      return true;
  ...
  if (more_than_one(diagonal_lines() & (to | targetRoyalSq)))
  {
      PieceType pt = type_of(moved_piece(m));
      PieceType diagType = pt == WAZIR ? FERS : pt == SOLDIER ? PAWN : pt == ROOK ? BISHOP : NO_PIECE_TYPE;
      if (diagType && (attacks_bb(sideToMove, diagType, to, occupied) & targetRoyalSq))
          return true;
  ...
  case PROMOTION:
      return attacks_bb(sideToMove, promotion_type(m), to, pieces() ^ from) & targetRoyalSq;

  case PIECE_PROMOTION:
      return attacks_bb(sideToMove, promoted_piece_type(type_of(moved_piece(m))), to, pieces() ^ from) & targetRoyalSq;

  case PIECE_DEMOTION:
      return attacks_bb(sideToMove, type_of(unpromoted_piece_on(from)), to, pieces() ^ from) & targetRoyalSq;
  ...
  case EN_PASSANT:
  {
      ...
      return attackers_to(targetRoyalSq, b) & pieces(sideToMove) & b;
  }
  default: //CASTLING
  {
      ...
      if (   castling_rank(WHITE) > RANK_1
          ...
          && attackers_to(targetRoyalSq, (pieces() ^ kfrom ^ rfrom) | rto | kto, sideToMove))
          return true;

      return   (PseudoAttacks[sideToMove][type_of(piece_on(rfrom))][rto] & targetRoyalSq)
            && (attacks_bb(sideToMove, type_of(piece_on(rfrom)), rto, (pieces() ^ kfrom ^ rfrom) | rto | kto) & targetRoyalSq);
  }
  }
}
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  const bool timurCitadelExchangeNativeMove = timur_is_citadel_exchange_native_move(*this, m);
  const bool timurRoyalSwapNativeMove = timur_is_royal_swap_native_move(*this, m);
  TimurPawnOfPawnsCyclePlan timurPawnOfPawnsCyclePlan = {
      SQ_NONE, SQ_NONE, WHITE, -1, -1, false, false
  };
  const bool timurPawnOfPawnsCycleNativeMove =
      timur_try_make_pawn_of_pawns_cycle_native_plan(*this, m, timurPawnOfPawnsCyclePlan);
  ...
  st = &newSt;
  st->move = m;
  st->timurCitadelExchange = false;
  st->timurCitadelExchangeTargetPiece = NO_PIECE;
  st->timurCitadelExchangeTargetUnpromotedPiece = NO_PIECE;
  st->timurCitadelExchangeTargetPromoted = false;
  st->timurRoyalSwap = false;
  st->timurRoyalSwapTargetPiece = NO_PIECE;
  st->timurRoyalSwapTargetUnpromotedPiece = NO_PIECE;
  st->timurRoyalSwapTargetPromoted = false;
  st->timurPawnOfPawnsCycle = false;
  st->timurPawnOfPawnsOriginalPiece = NO_PIECE;
  st->timurPawnOfPawnsOriginalUnpromotedPiece = NO_PIECE;
  st->timurPawnOfPawnsOriginalPromoted = false;
  st->timurPawnOfPawnsResultPiece = NO_PIECE;
  st->timurPawnOfPawnsPreviousStage = -1;
  st->timurPawnOfPawnsNextStage = -1;
  ...
  if (timurCitadelExchangeNativeMove)
  {
      Piece target = piece_on(to);

      assert(pc != NO_PIECE && target != NO_PIECE);
      assert(color_of(pc) == us && color_of(target) == us);
      assert(type_of(target) != KING);

      st->timurCitadelExchange = true;
      st->timurCitadelExchangeTargetPiece = target;
      st->timurCitadelExchangeTargetUnpromotedPiece = unpromoted_piece_on(to);
      st->timurCitadelExchangeTargetPromoted = is_promoted(to);
      ...
      if (!st->timurCitadelExchangeUsed[us])
      {
          st->timurCitadelExchangeUsed[us] = true;
          k ^= timur_citadel_exchange_used_key(us);
      }

      k ^= Zobrist::psq[pc][from] ^ Zobrist::psq[pc][to] ^ Zobrist::psq[target][to];

      if (type_of(target) == PAWN)
          st->pawnKey ^= Zobrist::psq[target][to];
      else
          st->nonPawnMaterial[us] -= PieceValue[MG][target];

      remove_piece(to);
      st->materialKey ^= Zobrist::psq[target][pieceCount[target]];
      move_piece(from, to);
  ...
  if (timurRoyalSwapNativeMove)
  {
      Piece target = piece_on(to);

      assert(pc == make_piece(us, KING));
      assert(target != NO_PIECE);
      assert(color_of(target) == us);
      assert(type_of(target) != KING);

      st->timurRoyalSwap = true;
      st->timurRoyalSwapTargetPiece = target;
      st->timurRoyalSwapTargetUnpromotedPiece = unpromoted_piece_on(to);
      st->timurRoyalSwapTargetPromoted = is_promoted(to);
      st->capturedPiece = NO_PIECE;
      st->capturedpromoted = false;
      st->unpromotedCapturedPiece = NO_PIECE;
      st->captureSquare = SQ_NONE;
      ...
      if (!st->timurRoyalSwapUsed[us])
      {
          st->timurRoyalSwapUsed[us] = true;
          k ^= timur_royal_swap_used_key(us);
      }

      k ^= Zobrist::psq[pc][from] ^ Zobrist::psq[pc][to]
         ^ Zobrist::psq[target][to] ^ Zobrist::psq[target][from];
      ...
      put_piece(target,
                from,
                st->timurRoyalSwapTargetPromoted,
                st->timurRoyalSwapTargetUnpromotedPiece);

      st->rule50 = 0;
      st->key = k;
      {
          const Square theirRoyalSq = timur_effective_royal_square(*this, them);
          st->checkersBB = theirRoyalSq != SQ_NONE ? attackers_to(theirRoyalSq, pieces(), us) & pieces(us) : Bitboard(0);
      }
      sideToMove = ~sideToMove;
      set_check_info(st);
  ...
  if (timurPawnOfPawnsCycleNativeMove)
  {
      const TimurPawnOfPawnsCyclePlan& plan = timurPawnOfPawnsCyclePlan;
      Piece result = plan.createsAdventitiousKing ? timur_adventitious_king_piece(us) : pc;

      assert(pc == make_piece(us, PAWN));
      assert(plan.from == from && plan.to == to);
      assert(plan.createsAdventitiousKing || empty(to));

      st->timurPawnOfPawnsCycle = true;
      st->timurPawnOfPawnsOriginalPiece = pc;
      st->timurPawnOfPawnsOriginalUnpromotedPiece = unpromoted_piece_on(from);
      st->timurPawnOfPawnsOriginalPromoted = is_promoted(from);
      st->timurPawnOfPawnsResultPiece = result;
      st->timurPawnOfPawnsPreviousStage = plan.previousStage;
      st->timurPawnOfPawnsNextStage = plan.nextStage;
      st->capturedPiece = NO_PIECE;
      st->capturedpromoted = false;
      st->unpromotedCapturedPiece = NO_PIECE;
      st->captureSquare = SQ_NONE;
      ...
      if (Eval::useNNUE)
      {
          dp.dirty_num = result == pc && from != to ? 1 : 2;
          dp.piece[0] = pc;
          dp.handPiece[0] = NO_PIECE;
          dp.from[0] = from;
          dp.to[0] = result == pc && from != to ? to : SQ_NONE;
          if (dp.dirty_num == 2)
          {
              dp.piece[1] = result;
              dp.handPiece[1] = NO_PIECE;
      ...
      if (plan.previousStage > TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL)
      {
          k ^= timur_pawn_of_pawns_stage_key(us, from, plan.previousStage);
          st->timurPawnOfPawnsStage[from] = TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL;
      }

      if (!plan.createsAdventitiousKing && plan.nextStage > TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL)
      {
          st->timurPawnOfPawnsStage[to] = plan.nextStage;
          k ^= timur_pawn_of_pawns_stage_key(us, to, plan.nextStage);
      }

      k ^= Zobrist::psq[pc][from] ^ Zobrist::psq[result][to];
      ...
      if (result != pc)
      {
          st->materialKey ^= Zobrist::psq[result][pieceCount[result] - 1]
  ...
  if (var->variantTemplate == "timur" && captured != NO_PIECE)
  {
      Square capturedStageSquare = type_of(m) == EN_PASSANT ? capture_square(to) : to;
      int capturedStage = st->timurPawnOfPawnsStage[capturedStageSquare];
      if (capturedStage > TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL && type_of(captured) == PAWN)
      {
          k ^= timur_pawn_of_pawns_stage_key(color_of(captured), capturedStageSquare, capturedStage);
          st->timurPawnOfPawnsStage[capturedStageSquare] = TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL;
      }
  }
  if (to == from)
  {
  ...
  if (type_of(pc) == PAWN)
  {
      int movingPawnOfPawnsStage = var->variantTemplate == "timur" && type_of(m) != DROP
                                 ? st->timurPawnOfPawnsStage[from]
                                 : TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL;
      if (movingPawnOfPawnsStage > TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL)
      {
          k ^= timur_pawn_of_pawns_stage_key(us, from, movingPawnOfPawnsStage);
          st->timurPawnOfPawnsStage[from] = TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL;

          if (type_of(m) != PROMOTION && type_of(m) != PIECE_PROMOTION)
          {
              st->timurPawnOfPawnsStage[to] = movingPawnOfPawnsStage;
              k ^= timur_pawn_of_pawns_stage_key(us, to, movingPawnOfPawnsStage);
          }
      }
...
void Position::undo_move(Move m) {
  ...
  assert(type_of(m) == DROP || empty(from) || st->timurRoyalSwap || st->timurPawnOfPawnsCycle || type_of(m) == CASTLING || is_gating(m)
         || (type_of(m) == PROMOTION && sittuyin_promotion())
         || (is_pass(m) && (pass(us) || var->wallOrMove)));
  assert(type_of(st->capturedPiece) != KING);

  if (st->timurCitadelExchange)
  {
      assert(type_of(m) == SPECIAL);
      assert(pc != NO_PIECE);
      assert(st->timurCitadelExchangeTargetPiece != NO_PIECE);
      assert(color_of(pc) == us);
      assert(color_of(st->timurCitadelExchangeTargetPiece) == us);

      move_piece(to, from);
      put_piece(st->timurCitadelExchangeTargetPiece,
                to,
                st->timurCitadelExchangeTargetPromoted,
                st->timurCitadelExchangeTargetUnpromotedPiece);
  ...
  if (st->timurRoyalSwap)
  {
      assert(pc == make_piece(us, KING));
      assert(st->timurRoyalSwapTargetPiece != NO_PIECE);
      assert(piece_on(from) == st->timurRoyalSwapTargetPiece);

      remove_piece(from);
      remove_piece(to);
      put_piece(pc, from);
      put_piece(st->timurRoyalSwapTargetPiece,
                to,
                st->timurRoyalSwapTargetPromoted,
                st->timurRoyalSwapTargetUnpromotedPiece);
  ...
  if (st->timurPawnOfPawnsCycle)
  {
      assert(st->timurPawnOfPawnsOriginalPiece != NO_PIECE);
      assert(pc == st->timurPawnOfPawnsResultPiece);

      remove_piece(to);
      put_piece(st->timurPawnOfPawnsOriginalPiece,
                from,
                st->timurPawnOfPawnsOriginalPromoted,
                st->timurPawnOfPawnsOriginalUnpromotedPiece);
...
Key Position::key_after(Move m) const {
  ...
  if (timur_is_royal_swap_native_move(*this, m))
  {
      if (!st->timurRoyalSwapUsed[sideToMove])
          k ^= timur_royal_swap_used_key(sideToMove);

      return k ^ Zobrist::psq[pc][from] ^ Zobrist::psq[pc][to]
               ^ Zobrist::psq[captured][to] ^ Zobrist::psq[captured][from];
  }

  TimurPawnOfPawnsCyclePlan pawnCyclePlan = { SQ_NONE, SQ_NONE, WHITE, -1, -1, false, false };
  if (timur_try_make_pawn_of_pawns_cycle_native_plan(*this, m, pawnCyclePlan))
  {
      Piece result = pawnCyclePlan.createsAdventitiousKing ? timur_adventitious_king_piece(sideToMove) : pc;
      if (pawnCyclePlan.previousStage > TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL)
          k ^= timur_pawn_of_pawns_stage_key(sideToMove, from, pawnCyclePlan.previousStage);
      if (!pawnCyclePlan.createsAdventitiousKing && pawnCyclePlan.nextStage > TIMUR_PAWN_OF_PAWNS_STAGE_INITIAL)
          k ^= timur_pawn_of_pawns_stage_key(sideToMove, to, pawnCyclePlan.nextStage);

      return k ^ Zobrist::psq[pc][from] ^ Zobrist::psq[result][to];
  }
...
bool Position::is_optional_game_end(Value& result, int ply, int countStarted) const {
  ...
  // Timur citadel entry: a royal that can enter the opposite citadel may claim a draw.
  if (var->variantTemplate == "timur" && timur_citadel_native_perft_root_token(*this)[0])
  {
      result = VALUE_DRAW;
      return true;
  }
...
bool Position::is_immediate_game_end(Value& result, int ply) const {
  ...
  // Timur has a royal hierarchy: Shah first, then Prince or Adventitious King
  // can keep the side alive. If none of those pieces exists, the side has lost.
  if (var->variantTemplate == "timur")
  {
      const bool usHasRoyal = timur_has_any_royal(*this, sideToMove);
      const bool themHasRoyal = timur_has_any_royal(*this, ~sideToMove);

      if (!usHasRoyal || !themHasRoyal)
      {
          result = !usHasRoyal && !themHasRoyal ? VALUE_DRAW
                 : !usHasRoyal                 ? mated_in(ply)
                                               : mate_in(ply);
          return true;
      }
  }
