...
namespace {
  ...
  Bitboard timur_staged_pawn_promotion_destinations(const Position& pos, Color us, Bitboard destinations) {
  ...
  ExtMove* generate_pawn_moves(const Position& pos, ExtMove* moveList, Bitboard target) {
    ...
    Bitboard timurBrcp = timur_staged_pawn_promotion_destinations<UpRight>(pos, Us, brcp);
    Bitboard timurBlcp = timur_staged_pawn_promotion_destinations<UpLeft>(pos, Us, blcp);
  ...
  ExtMove* generate_all(const Position& pos, ExtMove* moveList) {
    ...
    const Square ksq = timur_effective_royal_square(pos, Us);
    const PieceType effectiveRoyalType = timur_effective_royal_piece_type(pos, Us);
    const bool usesBackupRoyal = ksq != SQ_NONE && effectiveRoyalType != NO_PIECE_TYPE && effectiveRoyalType != KING;
    Bitboard target;
    ...
    if (Type != EVASIONS || !more_than_one(pos.checkers() & ~pos.non_sliding_riders()))
    {
        ...
        for (PieceSet ps = pos.piece_types() & ~(piece_set(PAWN) | KING); ps;)
        {
            ...
            if (Type == EVASIONS && usesBackupRoyal && pt == effectiveRoyalType)
                continue;
        ...
        if (Type == NON_EVASIONS)
        {
            for (int sq = SQ_A1; sq <= SQ_MAX; ++sq)
            {
                ...
                if (timur_try_make_citadel_exchange_native_plan(pos, targetRoyalFrom, plan)
                    && plan.color == Us)
                {
                    Move exchangeMove = timur_make_citadel_exchange_native_move(plan);
                    if (exchangeMove != MOVE_NONE)
                        *moveList++ = exchangeMove;
                }
            }

            for (int sq = SQ_A1; sq <= SQ_MAX; ++sq)
            {
                Square pawnFrom = Square(sq);
                if (!(pos.board_bb() & pawnFrom))
                    continue;

                TimurPawnOfPawnsCyclePlan plan;
                if (timur_try_make_pawn_of_pawns_cycle_native_plan(pos, pawnFrom, plan)
                    && plan.color == Us)
                {
                    Move cycleMove = timur_make_pawn_of_pawns_cycle_native_move(plan);
                    if (cycleMove != MOVE_NONE)
                        *moveList++ = cycleMove;
                }
            }
        }
        ...
        if (Type == EVASIONS)
        {
            for (int sq = SQ_A1; sq <= SQ_MAX; ++sq)
            {
                Square targetFrom = Square(sq);
                if (!(pos.board_bb() & targetFrom))
                    continue;

                TimurRoyalSwapPlan plan;
                if (timur_try_make_royal_swap_native_plan(pos, targetFrom, plan)
                    && plan.color == Us)
                {
                    Move royalSwapMove = timur_make_royal_swap_native_move(plan);
                    if (royalSwapMove != MOVE_NONE)
                        *moveList++ = royalSwapMove;
                }
            }
        }
    ...
    // Timur: once the Shah is gone, the effective backup royal must be able to
    // evade check like a king instead of being limited to block/capture targets.
    if (Type == EVASIONS && usesBackupRoyal)
    {
        Bitboard b = (  (pos.attacks_from(Us, effectiveRoyalType, ksq) & pos.pieces())
                      | (pos.moves_from(Us, effectiveRoyalType, ksq) & ~pos.pieces()))
                   & ~pos.pieces(Us)
                   & pos.board_bb();
        while (b)
            moveList = make_move_and_gating<NORMAL>(pos, moveList, Us, ksq, pop_lsb(b));
