...
namespace {
  ...
  uint64_t perft(Position& pos, Depth depth) {
    ...
    if (Root && depth <= 1)
    {
       const char* citadelToken = timur_citadel_native_perft_root_token(pos);
       if (citadelToken[0])
           sync_cout << citadelToken << ": " << 1 << sync_endl, nodes++;

       const std::string exchangeToken = timur_citadel_exchange_native_perft_root_token(pos);
       if (!exchangeToken.empty())
           sync_cout << exchangeToken << ": " << 1 << sync_endl, nodes++;

       const char* royalSwapToken = timur_royal_swap_native_perft_root_token(pos);
       if (royalSwapToken[0])
           sync_cout << royalSwapToken << ": " << 1 << sync_endl, nodes++;

       const char* pawnCycleToken = timur_pawn_of_pawns_cycle_native_perft_root_token(pos);
       if (pawnCycleToken[0])
           sync_cout << pawnCycleToken << ": " << 1 << sync_endl, nodes++;

       const char* multiRoyalToken = timur_multi_royal_native_perft_root_token(pos);
       if (multiRoyalToken[0])
           sync_cout << multiRoyalToken << ": " << 1 << sync_endl, nodes++;
    }
...
void MainThread::search() {
  ...
  if (rootPos.variant() && rootPos.variant()->variantTemplate == "timur")
  {
      const std::string exchangeToken = timur_citadel_exchange_native_perft_root_token(rootPos);
      if (!exchangeToken.empty())
      {
          const std::string exchangeEffect = timur_citadel_exchange_native_root_effect(rootPos);
          const std::string exchangeMoveEncoding = timur_citadel_exchange_native_move_encoding(rootPos);
          sync_cout << "info depth 0 score cp 0 string timur_special citadel_exchange" << sync_endl;
          if (!exchangeEffect.empty())
              sync_cout << "info string timur_special citadel_exchange " << exchangeEffect << sync_endl;
          if (!exchangeMoveEncoding.empty())
              sync_cout << "info string timur_special citadel_exchange encoded="
                        << exchangeMoveEncoding << " type=special" << sync_endl;
          sync_cout << "bestmove " << exchangeToken << sync_endl;
          return;
      }
  }

  Value rootOptionalResult;
  const bool rootOptionalGameEnd = rootPos.is_optional_game_end(rootOptionalResult);
  const bool timurRootOptionalGameEnd = rootOptionalGameEnd
                                      && rootPos.variant()
                                      && rootPos.variant()->variantTemplate == "timur";

  if (rootMoves.empty() || (CurrentProtocol == XBOARD && rootOptionalGameEnd) || timurRootOptionalGameEnd)
  {
      if (timurRootOptionalGameEnd)
          rootMoves.clear();
