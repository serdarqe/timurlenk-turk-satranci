...
const DEFAULT_MOVE_ANIMATION_MS = 140;
...
export class BoardRenderer {
    constructor(containerElement, gameState) {
        ...
        this._ensureTouchInteraction();

        this._applyPerspective();
    }

    _ensureTouchInteraction() {
        ...
        this.touchInteraction = new TouchInteraction(
            ...
            () => this._getCellSize(),
            {
                ...
                getPieceAt: (row, col) => this.gameState.board.getPieceAt(row, col),
                canDragPiece: (piece) => (
                    !this.isAnimating
                    && !this.gameState?.analysisPractice?.locked
                    && piece?.color === this.gameState.currentTurn
                    && this._canControlPiece(piece)
                ),
                onDragSelect: (row, col) => {
                    this.handleCellClick(row, col);
                    return this.selectedCell?.row === row && this.selectedCell?.col === col;
                },
                canDropOn: (row, col) => this.validMoves.some(move => move.row === row && move.col === col),
                onDragDrop: (row, col, dragContext) => this.handleCellClick(row, col, { dragContext }),
    ...
    render() {
        ...
        this._syncRoyalResultBadges();
        this._renderCapturedPieces();
    }

    _clearRoyalResultBadges() {
        this.royalResultBadgeHolders.forEach(holder => {
            holder?.classList?.remove('royal-result-holder', 'royal-result-winner', 'royal-result-loser');
            holder?.querySelectorAll?.('.royal-result-badge').forEach(badge => badge.remove());
        });
        this.royalResultBadgeHolders = [];

        [this.container, this.citadelAi, this.citadelPlayer].forEach(root => {
            root?.querySelectorAll?.('.royal-result-badge').forEach(badge => badge.remove());
            root?.querySelectorAll?.('.royal-result-holder').forEach(holder => {
                holder.classList.remove('royal-result-holder', 'royal-result-winner', 'royal-result-loser');
            });
        });
    }

    _getRoyalResultWinner() {
        const winner = this.gameState?.royalResultBadgeWinner || this.gameState?.winner;
        return winner === COLORS.WHITE || winner === COLORS.BLACK ? winner : null;
    }

    _findBadgeRoyalPiece(color) {
        const royals = (this.gameState?.board?.pieces || [])
            .filter(piece => piece?.color === color && GameRules.isRoyalType(piece.type))
            .sort((a, b) => {
                const rankDiff = GameRules.getRoyalRank(b.type) - GameRules.getRoyalRank(a.type);
                if (rankDiff !== 0) return rankDiff;
                if (a.type === PIECE_TYPES.KING && b.type !== PIECE_TYPES.KING) return -1;
                if (b.type === PIECE_TYPES.KING && a.type !== PIECE_TYPES.KING) return 1;
                return 0;
            });
        return royals[0] || null;
    }

    _addRoyalResultBadge(color, role) {
        const piece = this._findBadgeRoyalPiece(color);
        if (!piece) return;

        const holder = this._getCellElement(piece.row, piece.col);
        if (!holder) return;

        holder.classList.add('royal-result-holder', `royal-result-${role}`);
        holder.querySelectorAll?.('.royal-result-badge').forEach(badge => badge.remove());

        const badge = document.createElement('span');
        badge.className = `royal-result-badge is-${role}`;
        badge.textContent = role === 'winner' ? '✓' : '×';
        badge.setAttribute('role', 'img');
        badge.setAttribute(
            'aria-label',
            i18n.t(role === 'winner' ? 'game_end.royal_badge.winner' : 'game_end.royal_badge.loser')
        );
        holder.appendChild(badge);
        this.royalResultBadgeHolders.push(holder);
    }

    _syncRoyalResultBadges() {
        this._clearRoyalResultBadges();
        if (!this.gameState?.showRoyalResultBadges) return;

        const winner = this._getRoyalResultWinner();
        if (!winner) return;

        const loser = winner === COLORS.WHITE ? COLORS.BLACK : COLORS.WHITE;
        this._addRoyalResultBadge(winner, 'winner');
        this._addRoyalResultBadge(loser, 'loser');
    }
    ...
    _animateMove(fromRow, fromCol, toRow, toCol, hasCaptured, afterAnimationCallback, options = {}) {
        const dragContext = options?.dragContext || null;
        const cleanupDragContext = () => dragContext?.cleanup?.();
        ...
        const targetCenterX = destRect.left + (destRect.width / 2);
        const targetCenterY = destRect.top + (destRect.height / 2);
        const sourceCenterX = sourcePieceRect.left + (sourcePieceRect.width / 2);
        const sourceCenterY = sourcePieceRect.top + (sourcePieceRect.height / 2);
        const deltaX = targetCenterX - sourceCenterX;
        const deltaY = targetCenterY - sourceCenterY;
        const flyingPiece = sourcePiece.cloneNode(true);

        flyingPiece.classList.remove('selected', 'piece-drag-ghost');
        ...
        const durationMs = options?.durationMs ?? DEFAULT_MOVE_ANIMATION_MS;
        flyingPiece.style.transition = `transform ${durationMs}ms cubic-bezier(0.22, 1, 0.36, 1), filter ${durationMs}ms ease`;
        ...
        setTimeout(finishAnimation, durationMs + 90);
    }

    handleCellClick(row, col, options = {}) {
        const cleanupDragContext = () => options?.dragContext?.cleanup?.();
        if (this.isAnimating) {
            cleanupDragContext();
            return;
        }
        if (this.gameState?.analysisPractice?.locked) {
            cleanupDragContext();
            return;
        }
        ...
        if (this.selectedCell) {
            ...
            if (selectedMove) {
                ...
                }, { ...options, instant: Boolean(options?.dragContext?.instantDrop) });
                return;
            }
        }
