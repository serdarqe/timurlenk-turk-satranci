...
import { PIECE_SVGS_BOLD } from './PieceSVGs_Bold.js';
import { PIECE_SVGS_HERITAGE } from './PieceSVGs_Heritage.js';
...
function buildSVGMap(svgs) {
    return {
        ...
        [PIECE_TYPES.SEA_MONSTER]: svgs.SEA_MONSTER || svgs.VIZIER,
        [PIECE_TYPES.GENERAL]: svgs.GENERAL,
        [PIECE_TYPES.KNIGHT]: svgs.KNIGHT,
        [PIECE_TYPES.LION]: svgs.LION || svgs.KNIGHT,
        [PIECE_TYPES.ELEPHANT]: svgs.ELEPHANT,
        [PIECE_TYPES.CAMEL]: svgs.CAMEL,
        [PIECE_TYPES.DABBABA]: svgs.DABBABA,
        [PIECE_TYPES.BULL]: svgs.BULL || svgs.CAMEL,
        [PIECE_TYPES.REVEALER]: svgs.REVEALER || svgs.GENERAL,
...
function buildColorSVGMaps(svgsByColor) {
    return {
        white: buildSVGMap(svgsByColor.white),
        black: buildSVGMap(svgsByColor.black)
    };
}

const SVG_MAPS = {
    [PIECE_SKINS.HERITAGE]: buildColorSVGMaps(PIECE_SVGS_HERITAGE),
    [PIECE_SKINS.WARRIOR]: buildSVGMap(PIECE_SVGS_WARRIOR),
    [PIECE_SKINS.BOLD]: buildColorSVGMaps(PIECE_SVGS_BOLD)
};

function getCurrentSVGMap(color = null) {
    const skin = themeManager.getPieceSkin();
    const skinMap = SVG_MAPS[skin] || SVG_MAPS[PIECE_SKINS.BOLD];
    if (skinMap?.white || skinMap?.black) {
        return skinMap[color] || skinMap.white || skinMap.black || SVG_MAPS[PIECE_SKINS.BOLD].white;
    }
    return skinMap;
}
...
function buildPieceTemplate(type, color, pawnType, lang) {
    ...
    const svgMap = getCurrentSVGMap(color);
