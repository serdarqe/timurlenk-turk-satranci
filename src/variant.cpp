...
namespace {
    ...
    inline PieceType timur_custom_piece(int index) {
        return PieceType(CUSTOM_PIECE_1 + index);
    }

    // Tamerlane chess / Timur Satranci
    // Native built-in variant for the Timurlenk Turk Satranci fork.
    // This first native layer mirrors the validated JS/Fairy POC core:
    // 11x10 board, historical initial setup, core leapers/sliders and promotion.
    // Citadel exchange/draw, pawn subtype cycle and adventitious-king royal logic
    // are handled by the app wrapper until their dedicated C++ rule phase.
    // TIMUR_NATIVE_CITADEL_DRAW_SOURCE_MARKER
    // TIMUR_CITADEL_BLACK_OFFBOARD_JS_citadel_black_0_-1: off-board square left of a10.
    // TIMUR_CITADEL_WHITE_OFFBOARD_JS_citadel_white_9_11: off-board square right of k1.
    // TIMUR_CITADEL_DRAW_REQUIRES_native_offboard_result_apply_revert_smoke before native promotion.
    // TIMUR_NATIVE_CITADEL_EXCHANGE_SOURCE_MARKER
    // TIMUR_CITADEL_EXCHANGE_REQUIRES_native_dual_relocation_one_time_flag_apply_revert_smoke before native promotion.
    // TIMUR_CITADEL_EXCHANGE_SOURCE_GUARD_JS_authoritative_until_verified
    // TIMUR_NATIVE_ROYAL_SWAP_SOURCE_MARKER
    // TIMUR_ROYAL_SWAP_REQUIRES_native_check_escape_swap_one_time_flag_apply_revert_smoke before native promotion.
    // TIMUR_ROYAL_SWAP_SOURCE_GUARD_JS_authoritative_until_verified
    // TIMUR_NATIVE_PAWN_OF_PAWNS_SOURCE_MARKER
    // TIMUR_PAWN_OF_PAWNS_REQUIRES_native_stage_repatriation_apply_revert_smoke before native promotion.
    // TIMUR_PAWN_OF_PAWNS_SOURCE_GUARD_JS_authoritative_until_verified
    // TIMUR_CITADEL_NATIVE_MODEL_CONTRACT
    // TIMUR_CITADEL_OFFBOARD_NOT_BITBOARD: citadels stay outside the 11x10 board bitboard.
    // TIMUR_CITADEL_SPECIAL_MOVE_CHANNEL_REQUIRED: citadel entry must be carried as a special move, not a fake square.
    // TIMUR_CITADEL_PIPELINE_generate_apply_result_revert: generate -> apply -> result -> revert must be proven before native promotion.
    Variant* timur_variant() {
        Variant* v = chess_variant_base()->init();
        const PieceType DABBABA = timur_custom_piece(0);
        const PieceType CAMEL = timur_custom_piece(1);
        const PieceType SEA_MONSTER = timur_custom_piece(2);
        const PieceType LION = timur_custom_piece(3);
        const PieceType BULL = timur_custom_piece(4);
        const PieceType REVEALER = timur_custom_piece(5);
        const PieceType PRINCE = timur_custom_piece(6);
        const PieceType ADVENTITIOUS_KING = timur_custom_piece(7);
        const PieceType PICKET = timur_custom_piece(8);
        const PieceType GIRAFFE = timur_custom_piece(9);

        v->variantTemplate = "timur";
        v->maxRank = RANK_10;
        v->maxFile = FILE_K;
        v->reset_pieces();
        v->add_piece(PAWN, 'p');
        ...
        v->add_piece(ROOK, 'r');
        v->add_piece(KING, 'k');
        v->add_piece(WAZIR, 'v');              // Vizier: one orthogonal step.
        v->add_piece(FERS, 'g');               // General: one diagonal step.
        v->add_piece(ALFIL, 'e');              // Elephant: two-square diagonal leap.
        v->add_piece(DABBABA, 'd', "D");       // Two-square orthogonal leap.
        v->add_piece(CAMEL, 'c', "C");         // 3+1 leaper.
        v->add_piece(SEA_MONSTER, 's', "W");   // Same movement as vizier.
        v->add_piece(LION, 'l', "H");          // Historical long leaper approximation.
        v->add_piece(BULL, 'b', "Z");          // Zebra/bull leaper approximation.
        v->add_piece(REVEALER, 'h', "G");      // Revealer approximation.
        v->add_piece(PRINCE, 'q', "K");        // Prince moves as a king; royal state is wrapper phase.
        v->add_piece(ADVENTITIOUS_KING, 'a', "K");
        v->add_piece(PICKET, 't');             // Native Timur picket is generated in Position::moves_from().
        v->add_piece(GIRAFFE, 'z');            // Native Timur giraffe is generated in Position::moves_from().
        v->pieceValue[MG][GIRAFFE] = 980;
        v->pieceValue[EG][GIRAFFE] = 1040;

        v->startFen = "ecd5dce/rntzgkvztnr/ppppppppppp/11/11/11/11/PPPPPPPPPPP/RNTZGKVZTNR/ECD5DCE w - - 0 1";
        v->promotionRegion[WHITE] = Rank10BB;
        v->promotionRegion[BLACK] = Rank1BB;
        v->promotionPieceTypes[WHITE] = piece_set(WAZIR) | SEA_MONSTER | FERS | KNIGHT | LION | ALFIL | CAMEL | DABBABA | BULL | REVEALER | GIRAFFE | PICKET | ROOK | PRINCE;
        v->promotionPieceTypes[BLACK] = v->promotionPieceTypes[WHITE];
        v->mandatoryPawnPromotion = true;
        v->doubleStep = false;
        v->enPassantTypes[WHITE] = v->enPassantTypes[BLACK] = NO_PIECE_SET;
        v->castling = false;
        v->nFoldRule = 3;
        v->nMoveRule = 50;
        v->stalemateValue = VALUE_MATE;
...
void VariantMap::init() {
    ...
    add("timur", timur_variant());
    add("timur_poc", timur_variant());
