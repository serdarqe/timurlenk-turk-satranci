...
async function runPromotionSmoke() {
    ...
    try {
        ...
        const hasBasePromotionMove = promotionMoves.includes(fromTo);
        const nativeSuffixComplete = missingPromotionMoves.length === 0;
        const adapterGuardRequired = !nativeSuffixComplete && hasBasePromotionMove;

        return {
            ok: nativeSuffixComplete || adapterGuardRequired,
            variant,
            fromTo,
            fen: promotionFen,
            ...
            hasBasePromotionMove,
            nativeSuffixComplete,
            adapterGuardRequired,
            artifact: vendorRoot
        };
    } finally {
        try {
...
runPromotionSmoke()
    .then((result) => {
        if (jsonMode) {
            ...
            console.log(`Native suffix complete: ${result.nativeSuffixComplete ? 'yes' : 'no'}`);
            console.log(`Adapter guard required: ${result.adapterGuardRequired ? 'yes' : 'no'}`);
    ...
    .catch((error) => {
        if (jsonMode) {
            process.stdout.write(`${JSON.stringify({
                ...
                hasBasePromotionMove: false,
                nativeSuffixComplete: false,
                adapterGuardRequired: false
            })}\n`);
        } else {
